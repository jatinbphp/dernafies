...
export class ClientService 
{
	...
	constructor(public http: HttpClient, public router: Router, private alertCtrl: AlertController, private push: Push, private platform: Platform)
	{ 
	...
	changePassword(data)
	{
		...
		return new Promise((resolve, reject) => 
		{
			let dataToPost = new HttpParams().set("user_id",data.user_id).set("pwd",data.password);
			this.http.post(this.api_url + "updatePassword",  dataToPost , headers).subscribe((res: any) =>       
			{
				resolve(res);
			},
			err => 
